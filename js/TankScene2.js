class TankScene2 extends Phaser.Scene {
    ...
    create() {
        ...
        this.LayerWall = this.map.createLayer('wall', [landscape], 0, 0)
        this.LayerWall.setCollisionByProperty({collides:true})
        ...
        this.EnemyRemainingText = this.add.text(16, 16, 'Enemys Remaining: ' + this.maxEnemy, {
            fontSize: '20px',
            color: '#FFF'
        }).setScrollFactor(0)
        this.fuelText = this.add.text(16, 36, 'Fuel: ' + this.player.fuel +' metres Left', {
            fontSize: '20px',
            color: '#FFF'
        }).setScrollFactor(0)
        this.healthText = this.add.text(16, 56, 'Damage Taken: ' + this.player.damageCount+'/10',{
            fontSize: '20px',
            color: '#FFF'
        }).setScrollFactor(0)
    ...
    update(time, delta) {
        ...
        this.fuelText.setText('Fuel: '+this.player.fuel+' metres Left')
    ...
    createEnemy(dataObject){
        ...
        enemyTank.enableCollision(this.LayerWall)
    ...
    createPlayer(dataObject){
        ...
        this.player.enableCollision(this.LayerWall)
    }
    tryShoot(pointer){
    ...
    fireBullet(bullet, rotation, target){
        ...
        this.physics.add.collider(bullet, this.LayerWall,this.disposeOfBullet, null, this)
    ...
    bulletHitPlayer(hull, bullet){
        ...
        this.player.damage()
        this.healthText.setText('Damage Taken: '+this.player.damageCount+'/10')
    ...
    bulletHitEnemy(hull, bullet){
        ...
        if(enemy.isImmobilised()){
            ...
            if(enemy.isDestroyed()){
                ...
                this.maxEnemy -=1
                this.EnemyRemainingText.setText("Enemys Remaining:" + this.maxEnemy)
                if(this.maxEnemy === 0){
